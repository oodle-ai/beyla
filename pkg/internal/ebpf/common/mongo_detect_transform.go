...
func ProcessMongoEvent(buf []uint8, startTime int64, endTime int64, connInfo bpfConnectionInfoT, requests PendingMongoDBRequests) (*MongoRequestValue, bool, error) {
	...
	if !ok && isResponse {
		return nil, false, fmt.Errorf("no in-flight MongoDB request found for key %d (%d)", header.ResponseTo, requests.Len())
	}
...
func parseBodySection(buf []byte) (bson.D, int) {
	...
	// Try to parse the full BSON document first
	...
	// If full BSON parsing fails or we don't have enough data, try partial parsing
	// This is a best-effort attempt to extract key fields like operation name
	if len(buf) > int32Size {
		availableData := buf[int32Size:]
		partialDoc := tryParsePartialBson(availableData)
		if len(partialDoc) > 0 {
			return partialDoc, len(buf)
		}
	}
...
// tryParsePartialBson attempts to parse the beginning of a BSON document
// to extract key fields like operation name, even if the full document is truncated
func tryParsePartialBson(data []byte) bson.D {
	offset := 0
	doc := bson.D{}

	for offset < len(data) {
		// Need at least 2 bytes for type and at least 1 byte for field name
		...
		// Find the field name (null-terminated string)
		nameStart := offset
		nameEnd := -1
		...
		fieldName := string(data[nameStart:nameEnd])
		offset = nameEnd + 1
		...
		case 0x02: // String
			if offset+4 > len(data) {
				goto done // Not enough data for string length
			}
			strLen := int(binary.LittleEndian.Uint32(data[offset : offset+4]))
			offset += 4
			if offset+strLen > len(data) {
				goto done // Not enough data for string content
			}
			if strLen > 0 {
				// String includes null terminator in length
				strValue := string(data[offset : offset+strLen-1])
				doc = append(doc, bson.E{Key: fieldName, Value: strValue})
			}
			offset += strLen
		case 0x10: // Int32
			if offset+4 > len(data) {
			...
			doc = append(doc, bson.E{Key: fieldName, Value: int(intValue)})
			offset += 4
		case 0x12: // Int64
			if offset+8 > len(data) {
				goto done
			}
			intValue := int64(binary.LittleEndian.Uint64(data[offset : offset+8]))
			doc = append(doc, bson.E{Key: fieldName, Value: intValue})
			offset += 8
		case 0x01: // Double
			if offset+8 > len(data) {
				goto done
			...
			// Parse double value
			valueBytes := data[offset : offset+8]
			value := binary.LittleEndian.Uint64(valueBytes)
			floatValue := *(*float64)(unsafe.Pointer(&value))
			doc = append(doc, bson.E{Key: fieldName, Value: floatValue})
			offset += 8
		case 0x03: // Document
			if offset+4 > len(data) {
				goto done
			...
			// Skip document content for now, just advance offset
			offset += docLen
		case 0x04: // Array
			if offset+4 > len(data) {
				goto done
			}
			arrLen := int(binary.LittleEndian.Uint32(data[offset : offset+4]))
			if offset+arrLen > len(data) {
				goto done
			}
			// Skip array content for now, just advance offset
			offset += arrLen
		case 0x08: // Boolean
			if offset+1 > len(data) {
				goto done
			}
			boolValue := data[offset] != 0
			doc = append(doc, bson.E{Key: fieldName, Value: boolValue})
			offset += 1
		case 0x09: // DateTime
			if offset+8 > len(data) {
				goto done
			}
			// Skip datetime parsing for now, just advance offset
			offset += 8
		case 0x07: // ObjectId
			if offset+12 > len(data) {
				goto done
			}
			// Skip ObjectId parsing for now, just advance offset
			offset += 12
		case 0x11: // Timestamp (int64)
			if offset+8 > len(data) {
				goto done
			}
			// Skip timestamp parsing for now, just advance offset
			offset += 8
		case 0x13: // Decimal128
			if offset+16 > len(data) {
				goto done
			}
			// Skip decimal128 parsing for now, just advance offset
			offset += 16
		default:
			// For unknown types, try to skip them if we can determine their size
			// This is a best-effort approach to continue parsing

			// For unknown fixed-size types, we'll skip a reasonable amount and continue
			// This is not guaranteed to be correct but allows us to potentially find other fields

			// Try some common patterns for unknown types
			switch fieldType {
			case 0x05: // Binary data - has length prefix
				if offset+4 > len(data) {
					goto done
				}
				binLen := int(binary.LittleEndian.Uint32(data[offset : offset+4]))
				offset += 4 + 1 + binLen // length + subtype + data
			case 0x0A: // Null - no data
				// No additional data, continue
			case 0x0C: // DBPointer - deprecated, 4 bytes + 12 bytes ObjectId
				if offset+16 > len(data) {
					goto done
				}
				offset += 16
			case 0x0D, 0x0E, 0x0F: // JavaScript code types - length prefixed
				if offset+4 > len(data) {
					goto done
				}
				codeLen := int(binary.LittleEndian.Uint32(data[offset : offset+4]))
				offset += 4 + codeLen
			default:
				// If we can't determine the size, stop parsing to avoid corruption
				goto done
			}
		}

		// Stop after parsing a few fields to avoid infinite loops
		if len(doc) >= 15 {
			break
		}
	}
